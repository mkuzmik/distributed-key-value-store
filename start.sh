...
do
    case "$1" in
        ...
        "--replication" | "-r") replication="$2"; shift 2;;
...
do
    ...
    erl -kernel error_logger "{file, \"./logs/$counter@`hostname`.log\"}" -boot start_sasl -pa ebin -sname ${counter} -run store_app start ${port} ${masternode}@`hostname` ${replication} ${detached}
    ((counter++))
    ports="$ports $port"
    ((port++))
done
